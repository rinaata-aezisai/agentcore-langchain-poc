...
def compare_metrics(strands_data: dict, langchain_data: dict) -> list[ComparisonMetric]:
    ...
    l = langchain_data["summary"]
    
    # レイテンシ比較
    metrics.append(ComparisonMetric(
        metric_name="Average Latency (ms)",
        strands_value=s["avg_latency_ms"],
        langchain_value=l["avg_latency_ms"],
        difference=s["avg_latency_ms"] - l["avg_latency_ms"],
        winner="Strands" if s["avg_latency_ms"] < l["avg_latency_ms"] else "LangChain",
    ...
    metrics.append(ComparisonMetric(
        ...
        langchain_value=l["p50_latency_ms"],
        difference=s["p50_latency_ms"] - l["p50_latency_ms"],
        winner="Strands" if s["p50_latency_ms"] < l["p50_latency_ms"] else "LangChain",
        notes="Lower is better",
    ))
    ...
    metrics.append(ComparisonMetric(
        ...
        langchain_value=l["p95_latency_ms"],
        difference=s["p95_latency_ms"] - l["p95_latency_ms"],
        winner="Strands" if s["p95_latency_ms"] < l["p95_latency_ms"] else "LangChain",
        notes="Lower is better",
    ))
    ...
    metrics.append(ComparisonMetric(
        ...
        langchain_value=l["success_rate"],
        difference=s["success_rate"] - l["success_rate"],
        winner="Strands" if s["success_rate"] > l["success_rate"] else "LangChain",
...
def print_comparison_report(metrics: list[ComparisonMetric]):
    ...
    print(f"\n{'Metric':<25} {'Strands':<15} {'LangChain':<15} {'Diff':<15} {'Winner':<10}")
    print("-" * 80)
    ...
    for m in metrics:
        ...
        print(f"{m.metric_name:<25} {m.strands_value:<15.1f} {m.langchain_value:<15.1f} {diff_str:<15} {m.winner:<10}")
    ...
    if strands_wins > langchain_wins:
        ...
        print("   Recommendation: Use Strands Agents for AWS-native, low-latency applications")
    elif langchain_wins > strands_wins:
        print("\n🏆 Winner: LangChain + LangGraph")
        print("   Recommendation: Use LangChain for complex workflows and multi-agent systems")
    else:
...
This report compares the performance of AWS Bedrock AgentCore (Strands Agents) and LangChain + LangGraph implementations.
...
"""
    ...
    for m in metrics:
        report += f"| {m.metric_name} | {m.strands_value:.1f} | {m.langchain_value:.1f} | {m.difference:+.1f} | {m.winner} |\n"
