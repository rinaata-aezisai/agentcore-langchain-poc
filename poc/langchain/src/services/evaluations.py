"""LangChain Evaluations Adapter

LangSmith Evaluations / RAGAS による実装。
...
class LangChainEvaluationsAdapter(EvaluationsPort):
    """LangChain Evaluations (LangSmith/RAGAS) アダプター
    ...
    async def create_dataset(
        ...
        if self._langsmith_client:
            try:
                ls_dataset = self._langsmith_client.create_dataset(
                    dataset_name=name,
                    description=f"Evaluation dataset: {name}",
                )
                # ケースを追加
                for case in cases:
                    self._langsmith_client.create_example(
                        inputs={"input": case.input_data},
                        outputs={"output": case.expected_output} if case.expected_output else None,
    ...
    async def evaluate_with_llm_judge(
        ...
        # LangChain LLM評価
        score = await self._langchain_llm_evaluate(case, criteria, rubric)
        ...
        result = EvaluationResult(
            ...
            scores=[
                EvaluationScore(
                    ...
                    score=score,
                    details=f"LLM Judge (LangChain): {criteria}",
    ...
    async def _evaluate_by_type(
        ...
        if eval_type == EvaluationType.ACCURACY:
            ...
            score, details = await self._evaluate_relevance_ragas(case)
    ...
    async def _evaluate_relevance_ragas(self, case: EvaluationCase) -> tuple[float, str]:
        """RAGASスタイルの関連性評価"""
        # 本番ではRAGASライブラリを使用
        return 0.8, "関連性評価 (RAGAS style)"

    async def _evaluate_coherence(self, case: EvaluationCase) -> tuple[float, str]:
        """一貫性を評価"""
        return 0.75, "一貫性評価 (LangChain)"
    ...
    async def _langchain_llm_evaluate(
        self,
        case: EvaluationCase,
        criteria: str,
        rubric: str | None,
...
def create_langchain_evaluations() -> LangChainEvaluationsAdapter:
    """ファクトリ関数"""
    return LangChainEvaluationsAdapter()
